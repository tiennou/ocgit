...
NSString * const GTCLIDomainError = @"GTCLIDomainError";
...
static NSString *descForCredType(GTCredentialType type) {
    ...
    if (type & GTCredentialTypeSSHCustom) {
        [typeStrings addObject:@"ssh-custom"];
    }
    if (type & GTCredentialTypeSSHKey) {
...
+ (GTCredentialProvider *)credentialProviderWithArguments:(NSMutableArray *)arguments error:(NSError **)error {
    ...
    GTCredentialProvider *provider = [GTCredentialProvider providerWithBlock:^GTCredential *(GTCredentialType type, NSString *URL, NSString *userName) {
        ...
        if (type & GTCredentialTypeSSHKey && keyFileName != nil) {
...
+ (BOOL)executeToolWithArguments:(NSMutableArray *)arguments error:(NSError **)error {
    ...
    if (arguments.count == 0) {
        if (error) *error = [NSError errorWithDomain:GTCLIDomainError code:0 userInfo:@{
                                                                                        NSLocalizedDescriptionKey: @"No arguments passed"}];
        return NO;
    }
    ...
    if (tool == nil) {
        if (error) {
            NSString *desc = [NSString stringWithFormat:@"Unknown command %@", command];
            *error = [NSError errorWithDomain:GTCLIDomainError code:0 userInfo:@{NSLocalizedDescriptionKey: desc}];
        }
        return NO;
    }
